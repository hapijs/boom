...
HTTP is the basis for transmission of data over the internet. The [HTTP specification](https://en.wikipedia.org/wiki/Hypertext_Transfer_Protocol#Response_message) requires that a status code must be included in all http responses. This status code is made up of three digits and a short textual message. The **boom** library is used by the **[hapi](https://github.com/hapijs/hapijs.com)** web services framework to create all of its internal [http status codes](https://en.wikipedia.org/wiki/List_of_HTTP_status_codes). These messages can indicate problems, when this happens they are referred to as error messages. **Boom** gives the user a meaningful system to show these error messages. These error messages are customizable by changing the content of the key: `output`, which is the formatted response.  All errors returned by the **hapi** [reply interface](https://github.com/hapijs/hapi/blob/master/API.md#reply-interface) are converted to one of the **boom** objects listed below, or defaulted to status code 500 if it does not exist as a **boom** object.  
