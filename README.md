...
The **boom** library is used by hapi web services framework to create all of it's error codes. **Boom** gives the user a meaningful system to show HTTP errors. Error messages are customizable by changing the content of the key: `output`, which is the formatted response.  All errors returned by the **hapi** [reply interface](https://github.com/hapijs/hapi/blob/master/API.md#reply-interface) are converted to one of the **boom** objects listed below, or defaulted to status code 500 if it does not exist as a boom object.  
